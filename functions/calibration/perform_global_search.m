function [opt_phases, opt_velocity, min_err] = perform_global_search(parameters, profile_target, velocity)
    ...
    initial_guess = [randi(360, [1, parameters.transducer.elem_n]) / 180 * pi, velocity];
    lower_bounds = [zeros(1, parameters.transducer.elem_n), 0.001]; % Lower bounds: [0 rad, 1 mm/s]
    upper_bounds = [2 * pi * ones(1, parameters.transducer.elem_n), parameters.calibration.opt_upper_velocity]; % Upper bounds: [2pi rad, 200 mm/s]
