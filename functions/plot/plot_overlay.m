...
%   h               - Handle to the created figure.
    ...
    if ~isempty(trans_pos)
        ...
        [arc_x, arc_y] = getArc(geom_focus_pos, parameters.transducer(1).curv_radius_mm/grid_step, focal_angle-arc_halfangle, focal_angle+arc_halfangle );
    ...
    if options.show_rectangles 
        ...
        if ~isempty(trans_pos)
            ...
            rectangle('Position', [trans_pos(2)-parameters.transducer(1).pos_grid(3)-rect_size/2  trans_pos(1)-rect_size/2  rect_size*2+1 rect_size*2+1], 'EdgeColor', 'w', 'LineWidth',1,'LineStyle',':')
