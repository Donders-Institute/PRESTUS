...
% verbatim into manuscript Methods sections.
    ...
    % ------------------------------------------------------------------ %
    %  ITRUSST reporting note
    % ------------------------------------------------------------------ %
    txt = p(['Parameters are reported following the ITRUSST standardised '  ...
        'reporting guidelines for transcranial ultrasound stimulation '      ...
        '(' em('Martin et al., Brain Stimulation') ', 2024, 17:607&#8211;615; '  ...
        '<a href="https://doi.org/10.1016/j.brs.2024.04.013" '              ...
        'target="_blank" rel="noopener">DOI:10.1016/j.brs.2024.04.013</a>).']);
    ...
    txt = [txt p(sprintf([...
        ...
        'Simulations were executed using %s.'], ...
        b('Transducer and drive system.'),                                      ...
        fmt(n_elem, '%d', '?'), html_utils.escape(td_type),                          ...
        freq_khz, geom_str, html_utils.escape(code_type)))];
    ...
    txt = [txt p(sprintf([...
        ...
        'The medium was represented as %s.%s%s '                               ...
        'Tissue acoustic properties (sound speed, density, and power-law '     ...
        'attenuation coefficient) followed ITRUSST consensus values '          ...
        '(Aubry et al., 2025). '                                               ...
        'The method used to obtain ' em('in situ') ' exposure estimates '      ...
        'was full-wave acoustic simulation through the individual skull '       ...
        'and brain geometry, as recommended by Martin et al. (2024).'],        ...
        b(['Acoustic simulation and ' em('in situ') ' exposure estimates.']),   ...
        n_dims,                                                                 ...
        fmt(grid_res, '%.2g', '?'),                                            ...
        fmt(pml, '%d', '?'),                                                   ...
        medium_desc, seg_sentence, skull_sentence))]                           ;

    % ------------------------------------------------------------------ %
    %  3. Pulse timing parameters (ITRUSST Table 2.x format)
    % ------------------------------------------------------------------ %
    % Build the ITRUSST-format pulse timing table regardless of whether
    % thermal sims are enabled — the protocol is always defined.
    has_pulse_train = ~isnan(ptd_val) && ~isnan(ptri_val);

    timing_table  = '<table class="data-table timing-table">';
    timing_table  = [timing_table '<thead><tr>'                                ...
        '<th></th><th>Duration</th><th>Ramp duration</th>'                     ...
        '<th>Ramp shape</th><th>Repetition interval / PRF</th>'                ...
        '</tr></thead><tbody>'];

    % Row 1: Pulse
    pri_cell = iff(~isnan(pri_val) && ~isnan(pd_val), ...
        sprintf('%s&#8201;ms / %s&#8201;Hz', pri_ms, prf_hz), '—');
    timing_table = [timing_table sprintf([...
        '<tr><td><strong>Pulse</strong></td>'                                   ...
        '<td>%s&#8201;ms</td><td>—</td><td>rectangular</td><td>%s</td></tr>'], ...
        pd_ms, pri_cell)];

    % Row 2: Pulse train (if defined)
    if has_pulse_train
        ptri_cell = sprintf('%s&#8201;s', ptri_s);
        timing_table = [timing_table sprintf([...
            '<tr><td><strong>Pulse train</strong></td>'                         ...
            '<td>%s&#8201;s</td><td>—</td><td>rectangular</td><td>%s</td></tr>'], ...
            ptd_s, ptri_cell)];
    end
    timing_table = [timing_table '</tbody></table>'];

    dc_note = '';
    if ~strcmp(dc_pct, '?')
        dc_note = sprintf([' Pulse-level duty cycle (DC<sub>pulse</sub> = PD/PRI) '  ...
            '= %s&#37; (Martin et al., 2024, Eq.&#8201;1).'], dc_pct);
    end

    txt = [txt p(sprintf('%s Pulse timing parameters follow the ITRUSST convention (Martin et al., 2024):%s', ...
        b('Pulse timing parameters.'), dc_note)) timing_table];
    ...
    txt = [txt p(sprintf([...
        ...
        '(I<sub>SPPA</sub> = p&#178;<sub>sp</sub> / 2Z, where Z&#8201;=&#8201;1.5&#8201;&#215;&#8201;10&#8310;&#8201;Rayl '  ...
        'for soft tissue) and the transcranial mechanical index '               ...
        '(MI<sub>tc</sub> = p<sub>r,&#945;</sub> / &#8730;f<sub>0</sub>, '     ...
        'with f<sub>0</sub> in MHz) were derived from the simulated pressure '  ...
        'fields (Martin et al., 2024, Eqs.&#8201;7&#8211;8, 3). '              ...
        'Reported intensity values are pulse-average (I<sub>SPPA</sub>) '       ...
        'and should be read alongside the corresponding pressure values. '      ...
        'Non-significant risk limits followed the ITRUSST consensus on biophysical '     ...
        'safety (Aubry et al., 2025): '                                        ...
        'MI<sub>tc</sub> &#8804; 1.9; &#916;T &#8804; 2&#8201;&#176;C; '      ...
        'T<sub>peak</sub> &#8804; 39&#8201;&#176;C; '                          ...
        'CEM43 &#8804; 2&#8201;min (brain), &#8804; 16&#8201;min (skull), '    ...
        '&#8804; 21&#8201;min (skin).'],                                        ...
        b([em('In situ') ' exposure and safety metrics.'])))];
    ...
    txt = [txt p([...
        b('Output.') ...
        ' Three-dimensional maps of ' em('in situ') ' acoustic exposure — '    ...
        'I<sub>SPPA</sub>, MI<sub>tc</sub>' ...
        iff(run_thermal, ', temperature rise (&#916;T), and CEM43 thermal dose', '') ...
        ' — were exported as NIfTI volumes in subject-native space '           ...
        'and MNI-152 standard space.'])];

    % ------------------------------------------------------------------ %
    %  CC0 notice — identical framing to fMRIPrep
    % ------------------------------------------------------------------ %
    cc0 = ['<div class="boilerplate-license">'                                 ...
        ...
        'We fully intend for it to be copied verbatim into manuscripts, and '   ...
        'have released it under the '                                           ...
        '<a href="https://creativecommons.org/publicdomain/zero/1.0/" '        ...
        'target="_blank" rel="noopener">CC0 1.0 Universal</a> license, '       ...
        'dedicating it to the public domain. '                                  ...
        'Modifying the boilerplate '                                            ...
        em('will serve no legitimate scientific or literary purpose') ...
        ' and may reduce the replicability of the analysis being described.'    ...
        '</div>'];
