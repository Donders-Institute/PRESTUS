...
try
    ...
    % Verdict banner: overall pass/fail against ITRUSST NSR limits
    try
        html_parts{end+1} = build_safety_banner(csv_table, parameters, is_layered);
    catch ME
        html_parts{end+1} = html_utils.section_error('Safety verdict', ME);
    end

    % Section 1: Safety dashboard (ITRUSST-limited metrics only)
    try
    ...
    % Section 1b: Other (informational) exposure metrics
    try
        html_parts{end+1} = build_other_metrics(csv_table, parameters, is_layered);
    catch ME
    ...
    try
        html_parts{end+1} = html_utils.collapsible('Configuration Summary', ...
            [build_config_summary(parameters) ...
             '<h3>Full configuration (as used by prestus_pipeline)</h3>' ...
             build_config_dump(parameters)], false, 'config');
    catch ME
    ...
    html_parts{end+1} = '</footer>';
    % Lightbox overlay (markup) + shared interactive scripts (theme, search,
    % scroll-spy, meter fill, lightbox zoom/compare)
...
% ITRUSST safety section: only metrics with a finite NSR limit, split into a
% Mechanical (MI family) and a Thermal (temperature / dose) subgroup. The
% informational metrics live in build_other_metrics.
    limits = get_risk_limits(is_layered);
    metric_names = fieldnames(limits);
    [use_cem43_iso, cem43_iso_map] = local_cem43_iso(parameters);

    html = '<section class="report-section" id="safety">';
    html = [html '<h2>Safety &mdash; ITRUSST exposure</h2>'];
    ...
    if ~is_layered
        html = [html '<div class="medium-banner medium-water">' ...
            ...
            'See <a href="#other">Other exposure metrics</a> below.</div>'];
    end

    % Keep only ITRUSST-limited metrics, split into Mechanical (MI) / Thermal.
    mech_keys = {}; therm_keys = {};
    for i = 1:numel(metric_names)
        switch metric_group(metric_names{i}, limits.(metric_names{i}))
            case 'mechanical', mech_keys{end+1} = metric_names{i}; %#ok<AGROW>
            case 'thermal',    therm_keys{end+1} = metric_names{i}; %#ok<AGROW>
        end
    end
    html = [html render_dash_group(mech_keys, 'mech', 'Mechanical', 'mechanical index (MI) &amp; peak pressure', ...
        limits, csv_table, use_cem43_iso, cem43_iso_map)];
    html = [html render_dash_group(therm_keys, 'th', 'Thermal', 'temperature &amp; dose', ...
        limits, csv_table, use_cem43_iso, cem43_iso_map)];
...
function html = build_other_metrics(csv_table, parameters, is_layered)
% Informational exposure metrics (no ITRUSST NSR limit): TIC, ISPPA (global /
% per tissue) and peak pressure. Rendered as a flat grid in its own section.
    limits = get_risk_limits(is_layered);
    metric_names = fieldnames(limits);
    [use_iso, iso_map] = local_cem43_iso(parameters);
    other_keys = {};
    for i = 1:numel(metric_names)
...
function html = render_dash_group(keys, dom_class, title, subtitle, limits, csv_table, use_iso, iso_map)
% Render one labelled subgroup (acoustic or thermal) of safety tiles, each
% with a value, NSR meter + limit tick, hover tooltip, and over-limit flag.
    html = '';
    ...
    for i = 1:numel(keys)
        ...
        info = limits.(name);
        csv_name = name;
        if use_iso && isfield(iso_map, name)
            csv_name = iso_map.(name);
        end
        value = NaN; display_unit = info.unit;
        ...
        if strcmp(color, 'red')
            flagcls = ' flagged';
            flagbadge = '<span class="safety-flag">over</span>';
        end
        html = [html sprintf('<div class="safety-card safety-%s tip%s">', color, flagcls)]; %#ok<AGROW>
        html = [html sprintf('<div class="safety-label"><span>%s%s</span><span class="safety-q">?</span></div>', ...
            html_utils.escape(info.label), flagbadge)]; %#ok<AGROW>
        if isnan(value)
            html = [html '<div class="safety-value">N/A</div><div class="risk-limit">No data</div>']; %#ok<AGROW>
        else
            html = [html sprintf('<div class="safety-value">%.3g</div>', value)]; %#ok<AGROW>
            if isinf(info.limit)
                html = [html sprintf('<div class="risk-limit">%s informational</div>', html_utils.escape(display_unit))]; %#ok<AGROW>
            else
                html = [html sprintf('<div class="risk-limit">NSR limit: %.3g %s</div>', info.limit, html_utils.escape(display_unit))]; %#ok<AGROW>
                pct = min(100, max(0, (value / info.limit) * 100));
                html = [html sprintf(['<div class="safety-bar-track">' ...
                    '<div class="safety-bar" data-pct="%.0f" style="width:%.0f%%"></div>' ...
                    '<span class="safety-bar-limit" style="left:100%%"></span></div>'], pct, pct)]; %#ok<AGROW>
            end
        end
        html = [html sprintf('<div class="tip-pop">%s</div>', metric_tooltip(name, info))]; %#ok<AGROW>
        html = [html '</div>']; %#ok<AGROW>
    end
    html = [html '</div>'];
end

function g = metric_group(name, info)
% Classify a metric for the dashboard split:
%   'other'      — informational (no finite ITRUSST limit; e.g. TIC, ISPPA)
%   'mechanical' — MI family (transcranial / per-tissue mechanical index) or peak pressure
%   'thermal'    — temperature / dose ITRUSST metrics (riseT, riseT37, maxT, CEM43)
    if isinf(info.limit)
        g = 'other';
...
function [use_iso, iso_map] = local_cem43_iso(parameters)
% Whether to read the ISO CEM43 CSV variants, plus the column-name remap.
    use_iso = isfield(parameters, 'thermal') && isfield(parameters.thermal, 'cem43_iso') ...
        && parameters.thermal.cem43_iso == 1;
    iso_map = struct('CEM43_brain', 'CEM43iso_brain', ...
                     'CEM43_skull', 'CEM43iso_skull', ...
                     'CEM43_skin',  'CEM43iso_skin');
end

function s = metric_tooltip(name, info)
% Short definition shown on hover for each safety tile.
    switch name
        case 'MI_tc',    d = '<b>MI (transcranial)</b> &mdash; peak mechanical index over intracranial voxels (p<sub>neg</sub>/&radic;f).';
        case 'MI_brain', d = '<b>MI (brain)</b> &mdash; mechanical index within brain tissue.';
        case 'MI_skull', d = '<b>MI (skull)</b> &mdash; mechanical index within bone.';
        case 'MI_skin',  d = '<b>MI (skin)</b> &mdash; mechanical index within skin/scalp.';
        case 'TIC',      d = '<b>TIC</b> &mdash; cranial Thermal Index, W<sub>0</sub>/(40&middot;D<sub>eq</sub>) per IEC&nbsp;62359. Emitted-power estimate; informational.';
        case 'Isppa',       d = '<b>I<sub>SPPA</sub></b> &mdash; spatial-peak pulse-average intensity (global).';
        case 'Isppa_brain', d = '<b>I<sub>SPPA</sub> (brain)</b> &mdash; spatial-peak pulse-average intensity in brain.';
        case 'Isppa_skull', d = '<b>I<sub>SPPA</sub> (skull)</b> &mdash; spatial-peak pulse-average intensity in bone.';
        case 'Isppa_skin',  d = '<b>I<sub>SPPA</sub> (skin)</b> &mdash; spatial-peak pulse-average intensity in skin.';
        case 'Psptp',    d = '<b>P<sub>sptp</sub></b> &mdash; spatial-peak temporal-peak pressure.';
        case 'maxT_brain', d = '<b>T<sub>max</sub> (brain)</b> &mdash; peak absolute temperature in brain.';
        case 'maxT_skull', d = '<b>T<sub>max</sub> (skull)</b> &mdash; peak absolute temperature in bone.';
        case 'maxT_skin',  d = '<b>T<sub>max</sub> (skin)</b> &mdash; peak absolute temperature in skin.';
        case 'riseT_brain', d = '<b>&Delta;T (brain)</b> &mdash; rise above the per-tissue baseline (temp<sub>0</sub>).';
        case 'riseT_skull', d = '<b>&Delta;T (skull)</b> &mdash; rise above the per-tissue baseline (temp<sub>0</sub>).';
        case 'riseT_skin',  d = '<b>&Delta;T (skin)</b> &mdash; rise above the per-tissue baseline (temp<sub>0</sub>).';
        case 'riseT37_brain', d = '<b>&Delta;T from 37&deg;C (brain)</b> &mdash; absolute rise assuming a fixed 37&deg;C start (T<sub>max</sub>&minus;37).';
        case 'riseT37_skull', d = '<b>&Delta;T from 37&deg;C (skull)</b> &mdash; absolute rise assuming a fixed 37&deg;C start (T<sub>max</sub>&minus;37).';
        case 'riseT37_skin',  d = '<b>&Delta;T from 37&deg;C (skin)</b> &mdash; absolute rise assuming a fixed 37&deg;C start (T<sub>max</sub>&minus;37).';
        case 'CEM43_brain', d = '<b>CEM43 (brain)</b> &mdash; cumulative equivalent minutes at 43&deg;C.';
        case 'CEM43_skull', d = '<b>CEM43 (skull)</b> &mdash; cumulative thermal dose in bone.';
        case 'CEM43_skin',  d = '<b>CEM43 (skin)</b> &mdash; cumulative thermal dose in skin.';
        otherwise, d = sprintf('<b>%s</b>', html_utils.escape(info.label));
    end
    if isinf(info.limit)
        s = [d '<span class="lh">informational &mdash; no hard ITRUSST limit</span>'];
    else
        s = sprintf('%s<span class="lh">ITRUSST limit: %.3g %s</span>', d, info.limit, html_utils.escape(info.unit));
    end
end

function html = build_safety_banner(csv_table, parameters, is_layered)
% Overall pass/fail verdict against the finite NSR limits, with tallies.
    limits = get_risk_limits(is_layered);
    use_iso = isfield(parameters, 'thermal') && isfield(parameters.thermal, 'cem43_iso') ...
        && parameters.thermal.cem43_iso == 1;
    iso_map = struct('CEM43_brain','CEM43iso_brain','CEM43_skull','CEM43iso_skull','CEM43_skin','CEM43iso_skin');
    names = fieldnames(limits);
    nGreen = 0; nAmber = 0; nRed = 0; over_labels = {};
    for i = 1:numel(names)
        name = names{i}; info = limits.(name);
        if isinf(info.limit); continue; end   % informational metrics excluded from verdict
        csv_name = name;
        if use_iso && isfield(iso_map, name); csv_name = iso_map.(name); end
        if ~isempty(csv_table) && ismember(csv_name, csv_table.Properties.VariableNames)
            v = csv_table.(csv_name);
            if isnumeric(v) && ~isempty(v)
                switch risk_color(v(end), info.limit)
                    case 'red',   nRed = nRed + 1; over_labels{end+1} = info.label; %#ok<AGROW>
                    case 'amber', nAmber = nAmber + 1;
                    case 'green', nGreen = nGreen + 1;
                end
            end
        end
    end
    if nRed == 0
        ...
        title = 'Within ITRUSST non-significant-risk limits';
        sub = 'All evaluated exposure metrics are at or below their NSR limits.';
    else
        cls = 'fail';
        ...
        if nRed == 1
            title = '1 metric exceeds an NSR limit';
        else
            title = sprintf('%d metrics exceed an NSR limit', nRed);
        end
        sub = ['Exceeded: ' html_utils.escape(strjoin(over_labels, ', ')) '. Review the flagged tiles below.'];
    end
    html = sprintf('<div class="verdict %s">', cls);
    html = [html sprintf('<div class="glyph">%s</div>', glyph)];
    html = [html sprintf('<div><h2>%s</h2><p>%s</p></div>', html_utils.escape(title), sub)];
    html = [html '<div class="tally">'];
    html = [html sprintf('<div class="t g"><div class="n">%d</div><div class="l">Within</div></div>', nGreen)];
    html = [html sprintf('<div class="t a"><div class="n">%d</div><div class="l">Near</div></div>', nAmber)];
    html = [html sprintf('<div class="t r"><div class="n">%d</div><div class="l">Over</div></div>', nRed)];
    html = [html '</div></div>'];
end
...
% Quick-glance summary cards showing key simulation outcomes.
    ...
    % Always show: IPA at target, focal distance (transducer->peak) and the
    % exit-plane-referenced focal distance (EP->peak), then max ISPPA
    ...
    html = [html summary_card('EP-to-focus', csv_value(csv_table, 'ep_focal_distance_mm'), 'mm')];
...
function html = build_positioning_section(parameters, subject_id, affix)
    ...
    for t = 1:n_trans
        ...
        img_path = fullfile(parameters.io.dir_img, ...
            sprintf('sub-%03d_positioning%s%s.png', subject_id, affix, tnn_suffix));
...
function html = build_acoustic_section(csv_table, parameters, subject_id, medium, affix, is_layered)
    ...
    for t = 1:n_trans
        ...
        % Intensity on segmentation: acoustic_analysis writes per-slice
        % _x/_y/_z files for 3D grids and a single no-dim file for 2D. Probe
        % both (fix for the previously missing 3D segmentation overlay).
        html = [html html_utils.embed_image_dims(parameters.io.dir_img, ...
            sprintf('sub-%03d_%s_intensity', subject_id, medium), ...
            sprintf('%s%s', trans_suffix, affix), ...
            sprintf('Intensity on segmentation%s', trans_suffix), ...
...
function html = build_thermal_section(csv_table, parameters, subject_id, medium, affix, is_layered)
    ...
    thermal_images = {
        fullfile(td, sprintf('sub-%03d_%s_thermal%s.png',         subject_id, medium, affix)), 'Temperature vs time (focal)';
        fullfile(td, sprintf('sub-%03d_%s_thermalrise%s.png',     subject_id, medium, affix)), 'Temperature rise vs time (focal)';
        fullfile(td, sprintf('sub-%03d_%s_CEM%s.png',             subject_id, medium, affix)), 'CEM43 (kWave) vs time (focal)';
        fullfile(td, sprintf('sub-%03d_%s_CEM_iso%s.png',         subject_id, medium, affix)), 'CEM43 (ISO) vs time (focal)';
        fullfile(td, sprintf('sub-%03d_%s_thermal_max%s.png',     subject_id, medium, affix)), 'Temperature vs time (layer max)';
        fullfile(td, sprintf('sub-%03d_%s_thermalrise_max%s.png', subject_id, medium, affix)), 'Temperature rise vs time (layer max)';
        fullfile(td, sprintf('sub-%03d_%s_CEM_max%s.png',         subject_id, medium, affix)), 'CEM43 (kWave) vs time (layer max)';
        fullfile(td, sprintf('sub-%03d_%s_CEM_iso_max%s.png',     subject_id, medium, affix)), 'CEM43 (ISO) vs time (layer max)';
        fullfile(td, sprintf('sub-%03d_%s_thermal_protocol%s.png',subject_id, medium, affix)), 'Thermal protocol diagram';
    };

    html = [html '<div class="image-grid">'];
    % Max-temperature anatomical overlay: thermal_analysis writes per-slice
    % _x/_y/_z files for 3D grids and a single no-dim file for 2D. Probe both
    % (this is the fix for the previously missing heating overlay in 3D runs).
    html = [html html_utils.embed_image_dims(td, ...
        sprintf('sub-%03d_%s_maxT', subject_id, medium), affix, ...
        'Max temperature overlay', 'Max temperature overlay')];
...
% Find the most recent diary log file.
    ...
    pattern = sprintf('sub-%03d_%s_*%s.txt', subject_id, medium, affix);
...
% Build a sticky top bar: brand, status dot, nav links, search, theme toggle.
    html = '<nav class="toc" id="toc">';
    logo = html_utils.report_logo(26);
    if ~isempty(logo)
        html = [html '<a class="brand" href="#top">' logo '</a>'];
    else
        html = [html '<a class="brand" href="#top"><span class="mark" aria-hidden="true">' ...
            '<svg width="30" height="30" viewBox="0 0 40 40" fill="none">' ...
            '<path d="M4 20a16 16 0 0 1 32 0" stroke="var(--acoustic)" stroke-width="2" fill="none"/>' ...
            '<path d="M14 20a6 6 0 0 1 12 0" stroke="var(--thermal)" stroke-width="2" fill="none"/>' ...
            '<circle cx="20" cy="29" r="3" fill="var(--thermal)"/></svg></span>' ...
            '<span class="wm">PRE<b>STUS</b></span></a>'];
    end
    ...
    html = [html '<span class="toc-spacer"></span>'];
    html = [html '<label class="report-search hide-sm">' ...
    ...
    html = [html '<button class="theme-btn" id="theme-btn" type="button" title="Toggle dark mode" aria-label="Toggle theme">' ...
...
% Simulation-report CSS: now consolidated into css_styles_base(), the single
% design system shared with the group report. The legacy block below is kept
% unreachable for reference and may be removed later.
css = css_styles_base();
return; %#ok<UNRCH>
css = [css_styles_base() ...
newline ...
