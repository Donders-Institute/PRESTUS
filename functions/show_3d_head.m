...
%   None. The function generates a 3D visualization.
    ...
    if ~any(crop_at_target)
        ...
        if strcmp(parameters.transducer.element_shape, 'annular') || ~strcmp(parameters.transducer.element_shape.matrix.matrix_shape, 'dolphin')
            ...
            [totalMask] = setup_dolphin_mask(parameters.transducer);
            [transducer_mask, ~, parameters.transducer] = setup_dolphin_transducer_mask(size(segmented_img), parameters.transducer, trans_xyz, parameters.grid_step_mm, totalMask);
