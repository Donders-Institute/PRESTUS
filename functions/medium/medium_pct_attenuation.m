...
switch algorithm
    case 'k-plan'
        ...
        if parameters.medium.skull.alpha_coeff ~= kPlan_alpha || ...
            parameters.medium.skull.alpha_power ~= kPlan_alpha_power
            warning('Specified attenuation varies from k-Plan setup.')
        end
        alpha_coeff(skull_idx) = parameters.medium.skull.alpha_coeff;
        alpha_power(skull_idx) = parameters.medium.skull.alpha_power;
    ...
    case 'mueller'
        ...
        alpha_coeff(skull_idx) = alpha_pseudoCT(skull_idx)./(0.5^parameters.medium.skull.alpha_power);
