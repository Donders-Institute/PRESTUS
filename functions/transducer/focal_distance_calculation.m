...
%                     .pos_t1_grid        - (1×3 vector) Transducer position [voxels] in T1 grid (optional)
%                     .focus_pos_t1_grid  - (1×3 vector) Target focus position [voxels] in T1 grid (optional)
...
%   2. GEOMETRIC FALLBACK: For remaining unset transducers, compute Euclidean distance between .pos_t1_grid 
%      and .focus_pos_t1_grid in T1 space, scaled by voxel dimensions from NIfTI header
...
%   - T1 grid computation: ||focus_pos_t1_grid - pos_t1_grid|| × t1_grid_step_mm
...
%   parameters.transducer(2).pos_t1_grid = [100 120 80];     % Will auto-compute for secondary
%   parameters.transducer(2).focus_pos_t1_grid = [100 120 30];
