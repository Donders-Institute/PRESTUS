...
%
% See also: AFFINE_RESAMPLE_3D, AFFINE_APPLY_PTS, AFFINE_BOUNDS
...
arguments
    ...
    new_dims      = ceil(diff(affine_bounds(transformation_matrix,          [0 0 0; size(nii_image)])));
    previous_dims = ceil(diff(affine_bounds(previous_transformation_matrix, [0 0 0; size(nii_image)])));
    ...
    TF = transformation_matrix * translation_matrix';

    % Perform resampling using nearest-neighbor interpolation
    upsampled_image = affine_resample_3d(nii_image, TF, new_dims, 'nearest', 0);
    ...
    for i = 1:nT
        out_mat = round(affine_apply_pts([trans_pos_grid(i, :); focus_pos_grid(i, :)], TF));
