...
post_stim_steps_n = post_stim_period / parameters.thermal.sim_time_steps;
...
for trial_i = 1:parameters.thermal.n_trials
  ...
  for pulse_i = 1:on_off_repetitions
      ...
      kwaveDiffusion.takeTimeStep(on_steps_n, parameters.thermal.sim_time_steps);
      time_status_seq = [time_status_seq struct('time', num2cell(max([time_status_seq(:).time]) + (1:on_steps_n)*parameters.thermal.sim_time_steps), 'step', num2cell(max([time_status_seq(:).step]) + (1:on_steps_n)), 'status', "on")];
      % pulse off 
      if off_steps_n>0
          kwaveDiffusion.Q = 0;
          kwaveDiffusion.takeTimeStep(off_steps_n, parameters.thermal.sim_time_steps);
          time_status_seq = [time_status_seq struct('time', num2cell(max([time_status_seq(:).time]) + (1:off_steps_n)*parameters.thermal.sim_time_steps), 'step', num2cell(max([time_status_seq(:).step]) + (1:off_steps_n)), 'status', "off")];
      end
  end
  ...
  if post_stim_steps_n > 0 
      kwaveDiffusion.Q = 0;
      kwaveDiffusion.takeTimeStep(post_stim_steps_n , parameters.thermal.sim_time_steps);
      time_status_seq = [time_status_seq struct('time', num2cell(max([time_status_seq(:).time]) + (1:post_stim_steps_n)*parameters.thermal.sim_time_steps), 'step', num2cell(max([time_status_seq(:).step]) + (1:post_stim_steps_n)), 'status', "off")];
  end
end
