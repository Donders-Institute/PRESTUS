function [trans_pos, focus_pos, focus_pos_ras, trans_pos_ras] = position_transducer_localite(localite_instr_file, mri_hdr, ...
    parameters)
% position_transducer uses the data from the Localite instrument markers
% and MRI scan header to determine the transducer coordinate in voxel (ijk)
% space.
%
% - localite_instr_file is the path to the Localite instrument markers XML
% file
% - mri_hdr is the header of the MRI file that has the desired voxel space
% (used to extract world to voxel transformation matrix)
% - reference_transducer_distance is the distance between the reference used
% for the neuronavigation and the transducer, mm
% - focal_distance is the distance between the transducer and the focus
% point, mm
% 
% The Localite coordinates are in RAS format. For the helpful info on the NIFTI coordinates and headers, see the
% section "Orientation information" here https://brainder.org/2012/09/23/the-nifti-file-format/
% 
% The code for reading the Localite data is written by Judith Lefkes
