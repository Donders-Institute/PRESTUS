...
for ind = 1:size(diameter, 2)
    
    % get complex pressure for bowls with inner and outer aperature diam
    if diameter(1, ind) == 0
        p_el_outer = 0;
    else
        [~, ~, p_el_outer] = focusedBowlONeil_new(radius, diameter(1, ind), amplitude(ind), frequency, sound_speed, density, axial_position, []);
    end
    [~, ~, p_el_inner] = focusedBowlONeil_new(radius, diameter(2, ind), amplitude(ind), frequency, sound_speed, density, axial_position, []);
