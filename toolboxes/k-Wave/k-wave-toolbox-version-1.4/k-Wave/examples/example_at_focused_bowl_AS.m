% Modelling A Focused Bowl Transducer Assuming Axisymmetry Example
%
% This example models a focused bowl transducer assuming axisymmetry. The
% on-axis pressure is compared with the exact solution calculated using
% focusedBowlONeil.
%
...
lateral_size    = 45e-3/2;  % total grid size in the lateral dimension [m]
...
karray = kWaveArray('Axisymmetric', true, 'BLITolerance', bli_tolerance, 'UpsamplingRate', upsampling_rate);
...
switch model
    ...
    case 3
        ...
        % C++
        sensor_data = kspaceFirstOrderASC(kgrid, medium, source, sensor, input_args{:});        
...
% plot the source mask and grid weights (pml is outside the grid in this example)
figure;
subplot(1, 2, 1);
...
subplot(1, 2, 2);
imagesc(1e3 * (kgrid.y_vec - kgrid.y_vec(1)), 1e3 * kgrid.x_vec, grid_weights);
axis image;
xlabel('y [mm]');
ylabel('x [mm]');
