...
# Usage
# -----
#   bash setup_PRESTUS_env.sh [--no-plantus] [--no-deface] [--no-workbench]
#
#   --no-plantus    Skip nilearn / vtk / h5py (PlanTUS python deps)
#   --no-deface     Skip pydeface (MRI defacing deps)
...
for arg in "$@"; do
    case $arg in
        ...
        --no-deface)    INSTALL_DEFACE=false ;;
...
echo "[1/6] Downloading SimNIBS ${SIMNIBS_VERSION} environment spec..."
...
# ── 3. Install SimNIBS ────────────────────────────────────────────────────────
echo "[3/6] Installing SimNIBS ${SIMNIBS_VERSION}..."
...
if [ "${INSTALL_PLANTUS}" = true ]; then
    echo "[4/6] Installing PlanTUS dependencies (nilearn, vtk, h5py, pynput)..."
    conda run -n "${ENV_NAME}" pip install nilearn vtk h5py pynput
else
    echo "[4/6] Skipping PlanTUS dependencies (--no-plantus)."
fi

# ── 5. Install defacing dependencies (optional) ───────────────────────────────
if [ "${INSTALL_DEFACE}" = true ]; then
    echo "[5/6] Installing defacing dependencies (pydeface)..."
    conda run -n "${ENV_NAME}" pip install pydeface
else
    echo "[5/6] Skipping defacing dependencies (--no-deface)."
fi

# ── 6. Install Connectome Workbench (optional) ────────────────────────────────
if [ "${INSTALL_WORKBENCH}" = true ]; then
    echo "[6/6] Installing Connectome Workbench (wb_command)..."
    conda run -n "${ENV_NAME}" conda install -y -c conda-forge connectome-workbench
else
    echo "[6/6] Skipping Connectome Workbench (--no-workbench)."
...
echo "  python -c \"import simnibs; import nilearn; import pydeface; print('OK')\""
if [ "${INSTALL_WORKBENCH}" = true ]; then
